  php_fpm_{{ $version->safe }}:
    ...
    volumes:
      ...
      - ./storage/config/{{ $version->fpm_name }}/php.ini:/etc/php{{ $version->major }}/php.ini
      - ./storage/config/{{ $version->fpm_name }}/xdebug.ini:/etc/php{{ $version->major }}/conf.d/xdebug.ini
    environment:
